...
class GitHubClient:
    def __init__(self, token: Optional[str] = None):
        ...
        if self.token:
            self.client = Github(self.token, timeout=15)
        else:
            self.client = Github(timeout=15)

    async def get_repo_data(self, repo_url: str) -> Dict[str, Any]:
        def _sync_get():
            try:
    ...
    async def fetch_file_content(self, repo_url: str, path: str) -> str:
    ...
    async def get_file_content(self, file_path: str) -> str:
        for attempt in range(2):
            try:
                # Use the PyGithub repo object already initialized in a separate thread
                ...
                if isinstance(file_content, list):
                    ...
                    return ""
                decoded_bytes = await asyncio.to_thread(lambda: file_content.decoded_content)
                limit = 100 * 1024
